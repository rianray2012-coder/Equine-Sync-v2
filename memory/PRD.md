...
> **Status (2026-06-06): Phase 6B (Care Record State Guards) COMPLETE.** The two repeatable care-records mutations are now retry-safe (atomic, idempotent), response shapes unchanged. (1) `POST /feed-tasks/{id}/complete` is an **idempotent no-op on re-complete** — a conditional update (`completed:{$ne:True}`) preserves the original `completed_by`/`completed_at`; still returns the feed-task doc. (2) `POST /medication-logs` gains an **optional `client_log_id`**: when provided, an atomic upsert keyed by `(barn_id, client_log_id)` + `$setOnInsert` makes repeat posts return the first log (no duplicate, first-write-wins); when omitted, original insert behavior is unchanged. Status stays free-text (no allow-list in 6B). 6A's `_require_medication` check preserved. Boundary held: `routes/care.py` + care tests only — no task-engine/audit/billing/onboarding/permissions/frontend; Phase 5 frozen. New `tests/test_care_state_guards.py` (3 tests). Backend suite: **486 passed / 3 skipped** (483 + 3). Next (gated, plan-first): **6C** Care Search/Filtering Polish. See `docs/CARE_WORKFLOW_STRENGTHENING.md`.
