...
- **Recommended Action:** Create immutable `AuditLog` (see `DATA_MODEL.md`) + audit service + tests. **(Phase 5)** — ✅ **RESOLVED (Phase 5A–5D, 2026-06-06):** immutable, fail-open, barn-stamped, PII-redacting audit service (`core/audit.py` → `audit_log`) with full write-side instrumentation (auth/session, admin/destructive, barn/invite lifecycle, SR approve/decline, invoice paid, `permission.denied`) and a barn-scoped, role-gated read API (`GET /api/audit-logs`, `audit:read`). 36 audit tests (13 unit + 8 5B + 6 5C + 7 5D + 2 isolation-suite carries). Remaining (TTL/retention, tamper-evidence, IP on denials) tracked as **Phase 5E / backlog**. See `docs/AUDIT_LOGGING.md`.
...
- **Resolved (Security Patch 2E, 2026-05-31):** Closed three founder-beta findings surfaced in a GitHub security review. (a) **Destructive `POST /api/seed`** is now disabled by default via `ALLOW_SEED_ROUTE` (default `false`) and returns `404` when off; even when enabled it requires an authenticated **admin** in production, so production data can never be wiped anonymously (the internal startup auto-seed calls the seed logic directly and is unaffected). (b) **Registration privilege escalation** fixed: public `POST /api/auth/register` no longer accepts a client `role` (previously defaulted to `admin`) — it always creates a safe low-privilege `horse_owner`; privileged roles come only from admin invites / seed. (c) **Verification-token bypass** fixed: when `ENFORCE_EMAIL_VERIFICATION=true`, registration returns a `pending_verification` response with **no** access/refresh tokens (statelessly blocking unverified session use). Covered by `tests/test_security_patch_2e.py`.
- **Resolved (Security Patch 2E hardening, 2026-05-31):** Tightened the above. (a) **`POST /api/seed` is now blocked entirely in production** (returns `404` even with `ALLOW_SEED_ROUTE=true`); outside production it requires authenticated **admin** + an explicit `{"confirm":"SEED"}` body (pure decision in `core.config.evaluate_seed_access`). (b) **Startup auto-seed disabled in production** via `core.config.auto_seed_enabled` (`ALLOW_AUTO_SEED` override; default off in prod, on in dev/test) — a fresh production DB never silently creates demo accounts. (c) **Defense-in-depth verification gate** added inside both `get_current_user` implementations (`server.py` + `routes/auth.py`) via `core.config.user_verification_ok`: when enforcement is on, a pre-issued/old token for an unverified user is rejected with `403` (missing `email_verified` still treated as verified for legacy users). End-to-end proven (403 → verify → 200). Full suite **250 passed, 3 skipped**.
