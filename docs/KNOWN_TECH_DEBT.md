...
- **Recommended Action:** Create immutable `AuditLog` (see `DATA_MODEL.md`) + audit service + tests. **(Phase 5)** — 🟡 **PARTIALLY RESOLVED (Phase 5A, 2026-06-06):** immutable, fail-open, barn-stamped audit service shipped (`core/audit.py` → `audit_log` collection + startup indexes + 13 unit tests). PII redaction uses hardened normalized variant matching (collapses separators/camelCase, strips keys containing sensitive fragments like `token`/`secret`/`apikey`/`jwt`/`hash` + token-bearing URL keys) and includes an `ensure_audit_indexes()` spec test. Write-side route instrumentation (5B/5C) + read API (5D) pending. See `docs/AUDIT_LOGGING.md`.
