...
- **Still open:** Account-level brute-force lockout (MongoDB `login_attempts`, per the auth playbook) — **Phase 2D**; multi-replica rate-limit store (Redis) if/when horizontally scaled; frontend pages for reset/verify links (`/reset-password`, `/verify-email`).
