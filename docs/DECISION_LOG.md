...
- **Alternatives Considered:** Folding into `routes/reports.py` (rejected — reports.py is setup-health/nudges; a dedicated `digests.py` keeps concerns clean); moving `/owners` roster CRUD or the background schedulers (rejected per scope — `/owners` stays in `care.py`; schedulers + `ensure_digest_indexes` stay in `server.py` for 3G). HTTP routes and schedulers both delegate to the same `owner_digest.py` functions — single source of truth, unchanged.
- **Risks:** Low — mechanical move of 6 handlers + one `include_router` + a verified import trim (trimmed names are route-only; loops use a different subset). No schema/DB/index/frontend/permission changes. New `tests/test_digests_routes.py` (registration + auth/role gates + response shape). Existing owner tests (`test_owner_trust*`) unchanged and green. Full suite **279 passed, 3 skipped**.
- **Review Date:** Phase 3G (when schedulers + shared infra move).
- **Status:** Active

### 2026-06-02 — Phase 3D: Care/Task route consolidation (verification + closure)
- **Decision:** Closed Phase 3D as a **verification + documentation** phase rather than a route-move phase. Audit found that all task routes already live in `task_engine.py` and all care routes in `routes/care.py` — **nothing care/task-related remained inline in `server.py`** (only the 6 digest/recap routes, reserved for 3E). Therefore no production route logic was moved or changed. Concrete changes were limited to test hygiene + docs: (1) hardened the flaky `tests/test_dispatch_retry.py` (relaxed `assert n == 1` → `assert n >= 1`; the per-event `dispatch_attempts`/`dispatched_at` assertions on the specific event are retained, so dispatcher behavior is still proven); (2) added a lightweight `tests/test_task_routes_inventory.py` that asserts the expected task/care routes are registered (via the backend OpenAPI spec) and the GET lists are available under auth (and require auth).
- **Reason:** Avoid inventing churn. The modularization map's original 3D description ("ensure care/task endpoints live in care.py/task_engine; thin out server.py") predates 3A/3B; by 3D those routes were already modular. The honest deliverable is verification + a regression guard + closing the doc.
- **Alternatives Considered:** Restructuring `care.py`/`task_engine.py` for cohesion (rejected per approval — churn with no behavior benefit); merging legacy `feed_tasks` into the unified TaskEvent engine (rejected — that is feature work, explicitly out of scope; documented as an intentional separation); fetching OpenAPI via the external `/openapi.json` (rejected — that path routes to the frontend through the ingress; the guard reads the spec from the backend's internal port with a graceful skip).
- **Risks:** None to production — no route/path/response/auth/status/schema/DB/frontend changes. Test-only + docs. Full suite **274 passed, 3 skipped** (the previously-flaky dispatch test now stable under full-suite load).
- **Review Date:** Phase 3E.
- **Status:** Active
